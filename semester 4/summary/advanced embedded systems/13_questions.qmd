...
Verifikation entspricht dem Überprüfen von spezifischen Anforderungen. Validierung beschreibt den Prozess der Überprüfung der Kundenbedürfnisse.

Ein Beispiel, das den Unterschied zwischen Verifikation und Validierung verdeutlicht, ist die Entwicklung einer Website für einen Online-Shop. Die Verifikation würde sicherstellen, dass die Website den technischen Anforderungen entspricht, die für ihre korrekte Funktionsweise erforderlich sind. Dazu gehören Dinge wie das Testen von Links, die Überprüfung der Formulareingaben und die Sicherstellung, dass die Datenbank korrekt funktioniert. Wenn die Website all diese Tests besteht, gilt sie als verifiziert.

Die Validierung hingegen würde sicherstellen, dass die Website den Kundenbedürfnissen entspricht. Dies beinhaltet die Überprüfung, ob die Website leicht navigierbar ist, ob sie ein gutes Benutzererlebnis bietet, ob die Zahlungsmethoden für den Kunden bequem sind und ob das Design ansprechend ist. Wenn die Website diese Anforderungen erfüllt und eine positive Nutzererfahrung bietet, gilt sie als validiert.
...
Bei einem Refactoring wird etwas neu umgeschrieben oder ge¨andert, ohne die eigentliche Funktionalit¨at zu ¨andern. Ein Refactoring hat h¨aufig eine Verbesserung von Lesbarkeit und
