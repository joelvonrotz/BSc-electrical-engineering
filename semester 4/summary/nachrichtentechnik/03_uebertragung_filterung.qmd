...
![](images/verzerrungen/amplitude.png)

- Phasen- oder Laufzeitverzerrung: $\arg H(f) \neq -\omega\cdot t_d$
...
$P_x$, $P_y$ werden als Effektivwerte angegeben.
...
Die Elektrische Leistung in der **V***orwärtswelle* wird mit den Effektivwerten berechnet. Für die **R***ückwertswelle* wechselt das Vorzeichen der charakteristischen Impedanz und somit die Richtung des Leistungsflusses.
...
R_{rad}   & entspricht der abgestrahlten \textit{Wirkleistung}\\
...
Die Richtwirkung $D$ beträgt beim *isotropen Strahler* $1$. Bei einer *dipol-Antenne* gilt $D=1.64$. Allgemein wird die Richtwirkung über die maximale Leistungsdichte $S(\theta,\varphi)_{max}$ einer gerichteten Antenne bestimmt (wobei $\Delta\theta°_{-3dB}$ und $\Delta\varphi°_{-3dB}$ den Öffnungswinkel in der Horizontalen- respektive Vertikalen-Ebene beschreiben)
