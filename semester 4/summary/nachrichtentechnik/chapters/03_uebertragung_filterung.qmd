...
## Empirische Kanalmodell

$$
L_M = \left(4\pi \frac{d_0}{\lambda}\right)^2\cdot \left(\frac{r}{d_0}\right)^\gamma \qquad \text{oder}\qquad L_{MdB}=32.4+20\cdot\log_{10}(f_{[GHz]})+10\cdot\gamma\cdot\log_{10}(r_{r[m]})
$$

```{=latex}
\begin{conditions}
  \gamma & Parameter des Modells \\
...
[Rayleigh Fading Kanal]{.underline}:

$$
p_{u^2}(x)=\frac{1}{\overline{P}_r}\cdot e^{-\frac{x}{\overline{P}_r}}
$$

Wahrscheinlichkeit $P$ für das Unterschreiten einer Signalleistung $S_{min}$:

$$
P[u^2<S_{min}]=\int_0^{S_{min}}p_{u^2}(x)\ dx = \int_0^{S_{min}}\frac{1}{\overline{P}_r}\cdot e^{-\frac{x}{\overline{P}_r}}\ dx
$$