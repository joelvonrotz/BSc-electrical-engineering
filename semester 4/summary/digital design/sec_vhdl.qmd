...
VHDL steht für ***V**ery High Speed Integrated Circuit **H**ardware **D**escription **L**anguage*, klingt schnell und ist es auch. Diese Sprache dient für die Hardwarebeschreibung von FPGAs, insbesondere wie die "Logikfläche" konfiguriert wird, und ist [keine]{.underline} Programmiersprache.
:::
...
HW-nahes Design ermöglicht: \textcolor{OliveGreen}{\textbf{+}} kleinere, schnellere, energie-effizientere Schaltungen als ESL-Design \textcolor{BrickRed}{\textbf{-}} auf Kosten der Entwicklungszeit.
...
3.  Arbeitet alle Prozesse ab, die auf Signale sensitiv, deren Wert sich durch die aktuelle Transaktion geändert hat. Signalzuweisungen werden als zukünftige Transaktionen in die Event Queue abgelegt.
...
Eine Entity beschreibt den Komponenten für äusserliche Zugriffe $\rightarrow$ nur [Struktur des Komponents]{.underline} bekannt, aber [nicht]{.underline} den [Inhalt]{.underline}.
...
1.  Analog zu `#define` in C $\rightarrow$ werden während Kompilation eingefügt!
...
``` vhdl
type t_map is array(1 to 4) of std_logic; -- <1>
constant c_map : t_map := (1 => 'Z', 2 | 3 => '0', others => 1'); -- <2>
```
...
1.  Synthese übersetzt VHDL-Modell in eine RTL-Becshreibung aus Registern und kombinatorischen Blöcken
...
2.  **Optimzie** $\rightarrow$ Packt Primitives der synth. Netlist in Slices, Versuch Gatterkomb. zu vereinfachen (Reduktion HW)
